...
;(async () => {
  ...
  const ck = path.resolve(__dirname, 'tradingview-cookies.json');
  if (fs.existsSync(ck)) {
    const cookies = JSON.parse(fs.readFileSync(ck, 'utf8'));
  ...
  fs.writeFileSync(path.resolve(__dirname, 'tradingview_data.json'), JSON.stringify(data, null, 2));
